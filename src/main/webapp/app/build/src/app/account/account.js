...
.factory('sessionService', function($http, $base64) {
    ...
    session.login = function(data) {
        return $http.post("/carsim/login", "username=" + data.name +
        "&password=" + data.password, {
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        } ).then(function(data) {
            localStorage.setItem("session", {});
        }, function(data) {
            alert("error logging in with"+data.name);
        });
    };
...
.factory('accountService', function($resource) {
    ...
    service.register = function(account, success, failure) {
        var Account = $resource("/carsim/rest/accounts");
        Account.save({}, account, success, failure);
    };
    service.getAccountById = function(accountId) {
        var Account = $resource("/carsim/rest/accounts/:paramAccountId");
