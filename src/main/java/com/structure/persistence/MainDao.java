package com.structure.persistence;
...
import com.structure.exception.ModifiedEntityException;
import com.structure.exception.RemovedEntityException;
import com.structure.logger.LogUtil;
import com.structure.pagination.Page;
import com.structure.pagination.PageImpl;
import com.structure.pagination.PageRequest;
...
public class MainDao
{
    ...
    public final Page<BaseEntity> findByFilterByPagination(EntityManager manager, final PageRequest pageRequest, final BaseFilter filter) throws Exception
    {
        int totalNumberOfElements = findByFilter(manager, filter).size();

        int firstResult = (pageRequest.getPageNumber() - 1) * pageRequest.getPageSize();
        int maxResults = pageRequest.getPageSize();

        Query query = filter.createQuery(manager);
        query.setFirstResult(firstResult);
        query.setMaxResults(maxResults);

        List<BaseEntity> contents = query.getResultList();

        return new PageImpl<BaseEntity>(contents, pageRequest.getPageNumber(), pageRequest.getPageSize(), totalNumberOfElements);
    }

    public final Page<BaseEntity> findByFilterByPagination(EntityManager manager, final PageRequest pageRequest, final BaseFilter filter,
                                                           String queryMethod) throws Exception
    {
        int totalNumberOfElements = findByFilter(manager, filter, queryMethod).size();

        int firstResult = (pageRequest.getPageNumber() - 1) * pageRequest.getPageSize();
        int maxResults = pageRequest.getPageSize();

        Query query = createQueryForFilter(manager, filter, queryMethod);
        query.setFirstResult(firstResult);
        query.setMaxResults(maxResults);
